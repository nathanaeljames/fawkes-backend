...
def main():
    ...
    # The name of the tokenizer model, as specified in the model's config.json.
    # We will use this to find the tokenizer files in the local cache.
    tokenizer_model_name = "Qwen/Qwen3-1.7B"
    ...
    try:
        ...
        # Load the tokenizer from the local cache. By using the
        # `local_files_only=True` flag, we prevent any network access.
        # The `AutoTokenizer` will automatically find the files downloaded by
        # the `huggingface_hub` library.
        print(f"\nAttempting to load tokenizer for '{tokenizer_model_name}' from local cache...")
        tokenizer = AutoTokenizer.from_pretrained(tokenizer_model_name, trust_remote_code=True, local_files_only=True)
    ...
    # Retrieve the pad token ID as an integer from the tokenizer.
    # This value will be passed to the generate method to be used in padding.
