...
This script:
1. Updates existing speaker names (nathanael/jane)
...
def update_speaker_names(con):
    ...
    result = con.execute("""
        UPDATE speakers 
        SET firstname = 'Jane', surname = 'Doe'
        WHERE firstname = 'jane' AND surname = 'doe'
    """)
    print(f"Updated jane doe → Jane Doe: {result.fetchone()[0]} rows affected")
    ...
    results = con.execute("""
        ...
        FROM speakers 
        WHERE (firstname = 'Nathanael' AND surname = 'Warren')
           OR (firstname = 'Jane' AND surname = 'Doe')
    """).fetchall()
