    def extract_embedding_from_file(self, wav_path, sample_rate=None):
        ...
        try:
            ...
            with torch.no_grad():
                # First preprocess the audio to get spectral features
                ...
                # Now use the encoder with the preprocessed features
                encoder_output = self.model.encoder(audio_signal=processed_signal, length=processed_signal_length)
                
                # Handle tuple output from encoder
                if isinstance(encoder_output, tuple):
                    embeddings = encoder_output[0]  # Take the first element (embeddings)
                else:
    ...
    def extract_embedding_from_buffer(self, audio_int16, sample_rate=None):
        ...
        try:
            ...
            with torch.no_grad():
                # Use the encoder directly to get speaker embeddings from tensors
                # This bypasses the file-based get_embedding method
                embeddings = self.model.encoder(input_signal=audio_tensor, input_signal_length=audio_len)
