...
def main():
    ...
    tokenizer_cache_path = os.path.join(local_model_path, "tokenizer_cache")
    ...
    try:
        ...
        # Try to load the tokenizer from the local cache folder first
        print(f"\nAttempting to load tokenizer from local path: {tokenizer_cache_path}")
        try:
            tokenizer = AutoTokenizer.from_pretrained(tokenizer_cache_path, trust_remote_code=True)
            print("Tokenizer loaded successfully from local cache.")
        except Exception:
            # If local load fails, download and save it to the cache folder
            print(f"Local tokenizer not found or invalid. Downloading from '{llm_model_name}'...")
            tokenizer = AutoTokenizer.from_pretrained(llm_model_name, trust_remote_code=True)
            tokenizer.save_pretrained(tokenizer_cache_path)
            print(f"Tokenizer downloaded and saved to local path: {tokenizer_cache_path}")
