# second attempt at replacing Watson STT with Nemo
import asyncio
...
# Prepare streaming ASR helper
#STT_FRAME_LEN = 6400 # 0.4s chunks
#STT_TOTAL_BUFFER = 25600 # 1.6s buffer
''' INVALID OPTIONS
    frame_overlap=0.01,      # 10 ms
    sample_rate=16000,
    total_cache_size=8,
    decoder_delay_in_sec=0.32,
    use_hardware_decoder=True
'''
'''
frame_batch_asr = FrameBatchASR(
    asr_model=asr_model,
...
async def run_streaming_asr(client_id):
    stream = frame_batch_asr.create_stream()
    ...
    while True:
        try:
            ...
            stream.feed_audio_chunk(chunk)

            partial_hypothesis = stream.get_partial_hypothesis()
            if partial_hypothesis:
                print(f"[Client {client_id}] Interim: {partial_hypothesis.text}")
                await client_queues[client_id]["raw_asr"].put({"type": "interim", "text": partial_hypothesis.text})

            if stream.is_ready():
                final_hypothesis = stream.get_final_hypothesis()
                if final_hypothesis:
                    print(f"[Client {client_id}] Final: {final_hypothesis.text}")
                    await client_queues[client_id]["raw_asr"].put({"type": "final", "text": final_hypothesis.text})

        except asyncio.CancelledError:
            print(f"ASR stream cancelled for client {client_id}")
            break
        except Exception as e:
            print(f"ASR error for client {client_id}: {e}")
            break

    print(f"ASR stream ending for client {client_id}")
    stream.finalize()
