# Major restructuring of websockets server allowing for multiple clients, simultaneous bi-directional streaming using multi-threading
...
async def convert_audio_to_16k_pcm(audio_info: dict) -> bytes:
    """Converts various audio formats to 16kHz mono 16-bit PCM."""
    
    audio_type = audio_info.get("type")
    audio_data = audio_info.get("data")
    source_rate = audio_info.get("rate")  # only required for raw_pcm
    ...
    if audio_type == "wav":
        ...
        return await asyncio.to_thread(_convert_wav)

    elif audio_type == "raw_pcm":
        if source_rate is None:
            raise ValueError("Missing 'rate' for raw_pcm data.")
        
        def _convert_raw():
            return audioop.ratecv(audio_data, 2, 1, source_rate, 16000, None)[0]
        return await asyncio.to_thread(_convert_raw)
...
async def synthesize_xtts_audio(speaker_name, text, buffer):
    ...
    for chunk in chunks:
        ...
        chunk_np = chunk.cpu().numpy()  # safely move from CUDA to host
        converted_chunk = await asyncio.to_thread(prepare_for_streaming, chunk, 'float32', 22050)
        #audio_buffer = io.BytesIO()
        #sf.write(audio_buffer, chunk_np, samplerate=24000, format="WAV")
        #audio_buffer.seek(0)
        #yield buffer
        await buffer.put(converted_chunk)
