...
- Evaluation harness from day one with deterministic scoring per slice (task family): labeled routing verdicts, seeded recall facts with expected citations, fixed-corpus multi-hop citation ids, schema validation, loop/stop/latency observation; a reference-anchored LLM judge only for free-text slices; nightly smoke subset, full runs on fingerprint change and weekly; capability cards (versioned rows per model, quantization, serving config) read by the router and the external bridge; paid remote tiers scored on research-tier slices only, once per release and spot-checked monthly on a capped sample; panel-mode outcomes recorded as field observations on cards. Coding-patch slice added in Phase 3. [Phase 1]
