...
- Prompt builder (Layers 0-1 and the Layer 2 topic slice in text mode, each defined by what it carries and how it is fetched per Architecture Specification C-07; frozen-snapshot injection of the topic slice; local-time and elapsed-time rendering per session; silo assertion on every context item) + validator skeleton with bounded re-prompt of the proposer; structured-output JSON contract with a confidence field; FSM registry data structures.
...
Exit: two different speakers hold personalized conversations on separate devices; enrollment completes end-to-end; a fact stored yesterday is recalled with citation and correct local date today; voice-path pre-LLM overhead (Budget A) measured against the tentative ~150 ms target and a one-tool-round turn (Budget B) measured against the tentative ~500 ms target, both budgets revised from the measurements; time to first audio recorded for no-tool and one-tool turns; lifecycle walkthroughs for all ported FSMs green in CI.
...
- External API bridge: Claude escalation tool with local-first routing policy (confidence/complexity/criticality/explicit), remote tier selection from capability cards, OCR-first text/JSON/Markdown payloads, budget gate, per-call cost accounting, silo-stamped audit log; remote tiers scored once per model release and spot-checked monthly on a capped sample.
