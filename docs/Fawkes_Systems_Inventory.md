...
All subsystems, what each does, and the phase in which it is built. Phases refer to Fawkes_Implementation_Plan.md. "Carry-over" = exists from Iteration 1 and is retained. Updated 2026-09-26, v4 (2026-09-19 detail restored in full; 2026-09-20 and 2026-09-25 decisions applied; three-tier access model and review answers added; 2026-09-26 authentication answers applied). Cross-references use base file names.
...
| Identity & auth subsystem | Deferred identification tiers, recency-modulated thresholds, pass-blurb auth FSM (rotating pre-recorded passage pushed to the user's device and read aloud), retroactive turn attribution; user-to-silo membership with a default silo; enrollment as a workspace-scoped workflow creating an identity (provisional record, voiceprint, consent; the optional passphrase step is superseded by the pass-blurb) | Phase 2 |
| Device registration & workspaces | Devices paired with an issued device secret (never MAC address) and assigned to one workspace and conversation groups; per-workspace authorization of devices, apps, actions, and FSM functions; voice-approved administrator pairing for new devices and new voices; revocation; audit | Phase 2 (static tokens), Phase 5 (voice-approved pairing, web pairing UI) |
| Continuous identification & follow-me authorization | Per-utterance speaker identification driving per-utterance authorization; identification evidence pooled per conversation thread across the conversation group; Layer 1a after one positive identification, Layer 1b after three consecutive; writes to the speaker's own store on identification, reads on authentication; conversation threads across devices; user-level authorization inherited only within a conversation group; voice change drops a device to workspace level; conversation promotion to the user area on continuous identification | Phase 2 |
| Silo sessions | Live user session plus silo PIN (never the PIN alone; re-entry by PIN while the user session lasts); one device-exclusive session per silo asserted server-side on the WebSocket; idle lock and absolute lifetime configured separately from user sessions; other pipelines refuse silo data | Phase 5 |
...
| Structured-output contract | JSON schema for slot updates / transitions / tool calls / response_text / confidence; constrained decoding; response_text is a draft for transition-proposing turns — confirmation produced only after ratification; outputs ratified or rejected as a whole | Phase 1 |
...
| Subsystem | Purpose | Phase |
|---|---|---|
| Silo layer — Tier A | `workspaces` and `silos` tables; non-null scope kind and scope id on every content and derived row with `silo_id` required for silo-scoped rows; projects keyed `(scope_kind, scope_id, project_id)`; `silo_id` in every unique constraint and index prefix; single-silo tunnels and blocked pairs; per-silo content hash; immutable silo context on every MemoryStore call; export-and-re-ingest for cross-silo moves; permanent leakage tests. Projects may live in any scope (former Q-09 resolved 2026-09-25) | Phase 0-1 |
...
| ingest() contract | Single write path from Phase 0: typed idempotency (turns by runtime event id, documents by content hash per scope), provenance, scope/project stamps; success acknowledged only after the source row commits with derived jobs recorded in the same transaction; per-store fan-out handlers from the job table; turns, traces, and cost rows included | Phase 0-1 |
| Hybrid retrieval | Lexical + vector (Postgres full-text ranking; BM25 via extension only if the recall slice demands it — Q05), Reciprocal Rank Fusion, scope weighting within a silo and area weighting (silo → user → workspace); optional reranker; the whole utterance is the query (no keyword-selection step); deterministic sharpeners: ontology-entity dictionary match, previous-turn append | Phase 1-2 |
...
| External API bridge | Cloud disabled until a scoped policy is enabled; complexity as primary trigger, explicit request overriding the complexity gate but not the budget; per-request stateless context bundles with prompt caching (Claude as subagent, never session host); remote tier selection from capability cards; OCR-first text/JSON/Markdown payloads; budget gate; token/cost accounting with every remote turn stored locally; silo-stamped audit log; compression proxy later only if spend is material | Phase 3 |
