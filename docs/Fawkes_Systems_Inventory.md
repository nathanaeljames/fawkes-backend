...
| Identity & auth subsystem | Deferred identification tiers, recency-modulated thresholds, randomized challenge-phrase auth FSM, retroactive turn attribution; user-to-silo membership with a default silo; enrollment as a workspace-scoped workflow creating an identity (provisional record, voiceprint, consent, optional passphrase) | Phase 2 |
| Device registration & workspaces | Devices paired with a device-bound secret and assigned to one workspace and conversation groups; per-workspace authorization of devices, apps, actions, and FSM functions; revocation; audit | Phase 2 (voice devices), Phase 5 (web pairing UI) |
| Continuous identification & follow-me authorization | Per-utterance speaker identification driving per-utterance authorization; conversation threads across devices; user-level authorization inherited only within a conversation group; voice change drops a device to workspace level; conversation promotion to the user area on request | Phase 2 |
| Silo sessions | Web sign-in plus second factor; one device-exclusive session per silo asserted server-side on the WebSocket; idle lock and absolute lifetime configured separately from user sessions; other pipelines refuse silo data | Phase 5 |
